...
def loss_MAP(
    mu, tau, D, i0, i1, mu0, beta=1.0, gamma_shape=1.0, gamma_rate=1.0, alpha=1.0
):
    # mu0 = [0.0, 0.0]
    ...
    log_llh = (
        jnp.log(D)
        + log_tau_ij_inv
        - 0.5 * tau_ij_inv * (D ** 2 + d ** 2)
        + jnp.log(i0e(tau_ij_inv * D * d))
    )

    # index of points in prior
    si = list(set(i0 + i1))
    log_mu = multivariate_normal.logpdf(mu[si], mean=0.0, cov=1.0 / (beta * tau))
    log_tau = gamma.logpdf(tau[si], a=gamma_shape, scale=1.0 / gamma_rate)

    return jnp.sum(log_llh) + jnp.sum(log_mu + log_tau)
...
def lv_pmds2(
    ...
    for epoch in range(epochs):
        ...
        loss, [grads_mu, grads_tau] = loss_and_grads_MAP(
            mu, tau, dists, i0, i1, mu0, beta, gamma_shape, gamma_rate, alpha
        )
        mu = mu + lr * grads_mu
