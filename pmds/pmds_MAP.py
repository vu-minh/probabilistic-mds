# jax implementation of Probabilistic MDS with MAP
...
loss_and_grads_log_mu = jax.jit(
    jax.vmap(
        jax.value_and_grad(jax.jit(_log_prior_mu), argnums=[0]), in_axes=(0), out_axes=0
    )
)

loss_and_grads_log_ss = jax.jit(
    jax.vmap(
        jax.value_and_grad(jax.jit(_log_prior_ss), argnums=[0]), in_axes=(0), out_axes=0
    )
)
...
def pmds_MAP(
    ...
    for epoch in range(epochs):
        ...
        loss_lllh, grads_lllh = loss_and_grads_lllh(
        ...
        grads_ss = jnp.concatenate((lr * grads_lllh[2], lr * grads_lllh[3]), axis=0)
